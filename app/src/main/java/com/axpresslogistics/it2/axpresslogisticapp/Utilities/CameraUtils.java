...
import com.axpresslogistics.it2.axpresslogisticapp.acitvities.EmpProfileActivity;
...
public class CameraUtils {
    ...
    public static File getOutputMediaFile(int type) {
        ...
        File mediaStorageDir = new File(
                ...
                EmpProfileActivity.GALLERY_DIRECTORY_NAME);

        // Create the storage directory if it does not exist
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                Log.e(EmpProfileActivity.GALLERY_DIRECTORY_NAME, "Oops! Failed create "
                        + EmpProfileActivity.GALLERY_DIRECTORY_NAME + " directory");
                return null;
            }
        }
        ...
        File mediaFile;
        if (type == EmpProfileActivity.MEDIA_TYPE_IMAGE) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "IMG_" + timeStamp + "." + EmpProfileActivity.IMAGE_EXTENSION);
        } else {
            return null;
        }
