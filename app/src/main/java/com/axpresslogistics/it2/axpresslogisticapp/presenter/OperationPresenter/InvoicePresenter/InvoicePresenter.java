...
public class InvoicePresenter implements MainInteractor {
    private Map<String, String> header;
    private Map<String, String> body;
    private Gson gson;

    public InvoicePresenter(Map<String, String> header, Map<String, String> body) {
        this.header = header;
        this.body = body;
    ...
    public void loadItems(final LoadListener<Object> loadListener) {
        ...
        posApiInterface.invoiceRequest(header,body)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .unsubscribeOn(Schedulers.io())
                .subscribe(new Action1<InvoiceResponse>() {
                    @Override
                    public void call(InvoiceResponse invoiceResponse) {
                        if (invoiceResponse != null) {
                            //   ProjectUtil.customToast(context, "response.getStatus()");
                            Log.d("Status", invoiceResponse.getStatus());
                            loadListener.onSuccess(invoiceResponse);
                        }
                        else {
