...
public class ApprovalLeaveRequestPresenter implements MainInteractor {

    private Map<String, String> header;
    private Map<String, String> body;
    Gson gson;

    public ApprovalLeaveRequestPresenter(Map<String, String> header, Map<String, String> body) {
        this.header = header;
        this.body = body;
    ...
    public void loadItems(final LoadListener<Object> loadListener) {
        ...
        posApiInterface.leaveRequest(header, body)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .unsubscribeOn(Schedulers.io())
                .subscribe(new Action1<ApprovalLeaveResponse>() {
                    @Override
                    public void call(ApprovalLeaveResponse appliedLeavedResponse) {
                        if (appliedLeavedResponse != null) {

                            //   ProjectUtil.customToast(context, "response.getStatus()");
                            Log.d("Status", appliedLeavedResponse.getStatus() + appliedLeavedResponse.getSupervisiorEmplid());
