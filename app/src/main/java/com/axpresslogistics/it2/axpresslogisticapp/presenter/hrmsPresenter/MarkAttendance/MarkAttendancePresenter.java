...
public class MarkAttendancePresenter implements MainInteractor {
    Context context;
    ...
    public MarkAttendancePresenter(MarkAttendanceRequest markAttendanceRequest, Map<String, String> header) {
    ...
    public void loadItems(final LoadListener<Object> loadListener) {
        ...
        gson = new GsonBuilder().disableHtmlEscaping().create();
        final String json = gson.toJson(markAttendanceRequest);
        Log.e("request", json);

        posApiInterface.markattandance(header, markAttendanceRequest)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .unsubscribeOn(Schedulers.io())
                .subscribe(new Action1<MarkAttendanceResponse>() {
                    @Override
                    public void call(MarkAttendanceResponse response) {
                        if (response != null) {
